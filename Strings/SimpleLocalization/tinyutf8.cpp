...
utf8_string utf8_string::build_from_escaped(const char * strRaw)
{
	size_t lenRaw = strlen(strRaw);
	char * strCpy = new char[lenRaw + 1];
	memcpy(strCpy, strRaw, lenRaw);
	strCpy[lenRaw] = 0;
...
int operator-( const utf8_string::const_iterator& lhs , const utf8_string::const_iterator& rhs )
{
	...
	return max_index == lhs.get_index() ? static_cast<int>(num_codepoints) : -static_cast<int>(num_codepoints);
}
...
int operator-( const utf8_string::const_reverse_iterator& lhs , const utf8_string::const_reverse_iterator& rhs )
{
	...
	utf8_string::size_type			num_codepoints = lhs.get_instance()->get_num_resulting_codepoints( minIndex , max_index - minIndex );
	return max_index == rhs.get_index() ? static_cast<int>(num_codepoints) : -static_cast<int>(num_codepoints);
}
